...
- [x] `src/dmnbd.ipynb` を新規作成
- [x] import セル（os, pandas, numpy, matplotlib, seaborn, scipy.special.gammaln, scipy.optimize.minimize）

### Step 1: データ取得・前処理

- [x] Kaggle API で CSV ダウンロード（`transaction_data.csv`, `product.csv`）
- [x] 対象カテゴリの選定（下記「カテゴリ選定方針」参照）
- [x] カテゴリごとに household × 52週間フィルタ適用
...
- [x] `commodity_desc` ごとに上記集計を行い、対象カテゴリ一覧を確定する
- [x] 候補として SOFT DRINKS, CHEESE, FLUID MILK PRODUCTS, COLD CEREAL 等を検討
  - 選定: SOFT DRINKS, FLUID MILK PRODUCTS, CHEESE, BAG SNACKS, COLD CEREAL, YOGURT, FROZEN PIZZA, SOUP, FRZN MEAT/MEAT DINNERS, BAKED BREAD/BUNS/ROLLS（10カテゴリ）
...
- [x] 世帯ごとの購買回数 $r_i$ を集計
- [x] NBD の対数尤度を最大化して $(M, K)$ を推定
  - $M$: 平均購買回数、$K$: shape（小さいほどヘビーバイヤー偏在）
- [x] フィッティング結果を DataFrame `df_nbd` に格納（列: `category`, `M`, `K`）

### Step 4: カテゴリごとのディリクレ多項分布フィッティング

各カテゴリについて：

- [x] 購買回数上位 5 MANUFACTURER + Others の 6 ブランドに集約
- [x] 世帯 × ブランドのワイド形式 DataFrame を作成
- [x] ディリクレ多項分布の MLE で $\boldsymbol{\alpha}$ を推定、$S = \sum_j \alpha_j$ を算出
- [x] フィッティング結果を DataFrame `df_dm` に格納（列: `category`, `S`）

### Step 5: $K$ × $S$ マッピング

- [x] `df_nbd` と `df_dm` を `category` キーで結合
- [x] 散布図を作成（x 軸: $K$、y 軸: $S$、各点にカテゴリ名を付記）→ `article/fig_ks_map.png`
- [x] 4象限の解釈をアノテーションで追加
