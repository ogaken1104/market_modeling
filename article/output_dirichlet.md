...
ブランドごとのリピート率は、購買データから直接計算することもできる。しかし単純集計には本質的な問題がある。

- **ノイズの問題**: 購買回数が少ない消費者（例：2回しか買っていない人）のリピート率は 0% か 100% しかなく、個人レベルの推定値は極めて不安定である
- **市場構造が見えない**: 消費者ごとに独立にリピート率を計算しても、「このカテゴリはロイヤルティが高い市場か」という問いには答えにくい

ディリクレ多項分布モデルは、これらの問題に対して構造的な解決を与える。

- **プーリング**: 全消費者のデータを $\boldsymbol{\alpha}$ という共通パラメータに集約して推定する。購買回数が少ない消費者も、市場全体のパラメータ推定に貢献する
- **市場の要約**: $S$ という1つの数値でカテゴリ全体のロイヤルティを要約できる。異なるカテゴリ間での比較が直接可能になる
- **生成モデル**: パラメータが推定できれば「次の購買がどうなるか」のシミュレーションができる。施策の効果予測やシナリオ分析に使える
...
NBD の $K$ パラメータ（購入頻度の異質性）とディリクレの $S$ パラメータ（ブランドロイヤルティ）を組み合わせると、市場を二次元で分類できる。この分析は複数カテゴリに両モデルを当てはめ、カテゴリをプロット上に配置することで初めて意味を持つ。詳細は次稿で扱う。
